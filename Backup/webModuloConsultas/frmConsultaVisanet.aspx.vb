...
Public Class frmConsultaVisanet
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Not IsPostBack Then
            ...
            subConsultaEmpresas()
            modConfig.loadConfigVisanet(iIdEmpresa, sMensaje)
    ...
    End Sub
    Protected Sub subLoadCmbEmpresas()
        Dim lista As ClsListaEmpresa
        Dim oEmpresa As ClsEmpresa

        lista = Session(SES_LISTEMPRESA)
        Dim i As Integer
        Dim iServicio As System.Web.UI.WebControls.ListItem
        For i = 0 To lista.Count - 1
            oEmpresa = lista.getItem(i)
            iServicio = New System.Web.UI.WebControls.ListItem
            iServicio.Text = oEmpresa.getName
            iServicio.Value = oEmpresa.getId

            Me.cmbEmpresa.Items.Add(iServicio)

        Next

    End Sub
    Protected Sub subConsultaTranVisanet(Optional ByVal bExportar As Boolean = False)
    ...
    Protected Function subArmaQuery() As String
        ...
        If Me.txtNumTerminal.Text <> "" Then
            ...
            Else
                sWhere = sWhere & " AND"
            End If
            sWhere = sWhere & " LOGPO_NRO_POS = '" & Me.txtNumTerminal.Text & "'"
        End If

        If Me.txtIdTran.Text <> "" Then
            If Trim(sWhere) = "" Then
                sWhere = sWhere & " WHERE"
            Else
                sWhere = sWhere & " AND"
            End If
            sWhere = sWhere & " LOGPO_ID_TRANSACCION = '" & Me.txtIdTran.Text & "'"
        End If
        ...
        sValor = Me.cmbEmpresa.SelectedValue
        If sValor <> "" And sValor <> sCodTODOS Then
            If Trim(sWhere) = "" Then
                sWhere = sWhere & " WHERE"
            Else
                sWhere = sWhere & " AND"
            End If
            sWhere = sWhere & " LOGPO_CMCIO_ID = " & CStr(CInt(Me.cmbEmpresa.SelectedValue)) & ""
        End If
    ...
    Protected Sub subConsultaServicios()
        ...
        End If
    End Sub
    Protected Sub subConsultaEmpresas()
        ...
        If CN.State = ConnectionState.Open Then
            ...
            ds = cConsulta.spQRYEmpresa(CN, 0, "", sEstadoActivo, 0, iIdUsuario, sCR, sDescError)
            If sCR = TODO_OK Then
                ...
                Else
                    ...
                    'CARGA LAS EMPRESAS EN LA CLISTA DE SERVICIOS
                    loadListEmpresa(ds, sMensaje, True)
                End If
    ...
    Protected Sub loadListServicio(ByVal ds As DataSet, ByRef sMensaje As String, Optional ByVal bTodos As Boolean = False)
        ...
        Try
            ...
            For i = 0 To ds.Tables(0).Rows.Count - 1
                ...
                Session(SES_LISTSERVICIO) = cListServicios
            Next
    ...
    Protected Sub loadListEmpresa(ByVal ds As DataSet, ByRef sMensaje As String, Optional ByVal bTodos As Boolean = False)

        Dim empresa As ClsEmpresa
        Dim i As Integer
        Try

            modConfig.cListEmpresas = New ClsListaEmpresa
            If bTodos Then
                empresa = New ClsEmpresa
                empresa.setId(sCodTODOS)
                empresa.setName(sTodos)
                cListEmpresas.Add(empresa)
            End If
            For i = 0 To ds.Tables(0).Rows.Count - 1
                empresa = New ClsEmpresa
                ...
                'service.setId(ds.Tables(0).)
                'service.setName(.GetNamedItem("name").Value)
                empresa.setId(ds.Tables(0).Rows(i).Item("CMCIO_ID"))
                empresa.setName(ds.Tables(0).Rows(i).Item("CMCIO_NOMBRE"))

                cListEmpresas.Add(empresa)

                Session(SES_LISTEMPRESA) = cListEmpresas
            Next
    ...
    Protected Sub gridTran_SelectedIndexChanged(ByVal sender As Object, ByVal e As EventArgs) Handles gridTran.SelectedIndexChanged
