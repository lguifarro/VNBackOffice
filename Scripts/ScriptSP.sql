...
SELECT	UC.USUARIO_CODIGO,  UC.CMCIO_ID, U.USUARIO_PASSWORD, CO.CMCIO_NOMBRE
...
OPEN curUsuario;
fetch curUsuario 
into sCodUsuario, iIdComercio, sPassword, sNomComercio;
...
@
COMMENT ON  SPECIFIC PROCEDURE "SQMNUC".spLOGINWeb IS 'Login para el modulo de consultas web'
@
...
DECLARE curUsuario CURSOR FOR
SELECT	UG.IDUSUARIO, UG.PASSWORD, UG.IDGRUPO, UG.NOMBRE, UG.APELLIDOP, UG.EMAIL, COALESCE(GC.NOMBRE_GRUPO,'')
...
OPEN curUsuario;
fetch curUsuario 
into iIdUsuario, sPassword, iIdGrupo, sNombre, sApellidoP, sEmail, sNomGrupo;
...
IF EXISTS (SELECT COD_GRUPO FROM SQMNUC.GRUPO_COMERCIO WHERE COD_GRUPO = PCODGRUPO) THEN
...
ELSE
	INSERT INTO SQMNUC.GRUPO_COMERCIO (COD_GRUPO, NOMBRE_GRUPO, DESC_GRUPO, FLAG_RUC, RUC, FECHAINSERT, FECHAULTACTUALIZ)
	VALUES
	(PCODGRUPO, PNOMGRUPO, PDESCGRUPO, PFLAGRUC, PRUC, CURRENT TIMESTAMP, CURRENT TIMESTAMP);
...
DECLARE CURUSUARIO CURSOR FOR 
	SELECT IDGRUPO, COD_GRUPO, NOMBRE_GRUPO, FLAG_RUC, RUC, DESC_GRUPO, FECHAINSERT, FECHAULTACTUALIZ 
	FROM SQMNUC.GRUPO_COMERCIO
	WHERE IDGRUPO = PIDGRUPOCONSULTA;

-- Cursor left open for client application.
	OPEN CURUSUARIO;
	FETCH CURUSUARIO INTO
	iIdGrupo, sCodGrupo, sNomGrupo, iFlagRUC, iRUC, sDescGrupo, dFechaInsert, dFechaUltActualiz; 
...
SET V_SQL_SELECT = 'SELECT IDGRUPO, COD_GRUPO, NOMBRE_GRUPO, FLAG_RUC, RUC, DESC_GRUPO FROM SQMNUC.GRUPO_COMERCIO';
...
SET V_SQL_SELECT = 'SELECT U.IDUSUARIO, U.IDGRUPO, G.NOMBRE_GRUPO, U.LOGIN, U.PASSWORD, U.NOMBRE, U.APELLIDOP, U.EMAIL, U.ESTADO FROM SQMNUC.USUARIO_COMERCIO_CONS U, SQMNUC.GRUPO_COMERCIO G ';

	SET V_SQL_WHERE = ' WHERE U.IDGRUPO = G.IDGRUPO ';
	...
	IF RTRIM(COALESCE(PPIDGRUPO,0)) <>0 THEN
		IF V_SQL_WHERE ='' THEN
			SET V_SQL_WHERE = V_SQL_WHERE || ' WHERE';
		ELSE
			SET V_SQL_WHERE = V_SQL_WHERE || ' AND';
		...
		SET V_SQL_WHERE = V_SQL_WHERE || ' U.IDGRUPO =' || RTRIM(CAST(CAST(PPIDGRUPO AS CHAR(3)) AS VARCHAR(3))) || '';
	END IF;
